...
class DownloadsPT
{
	...
	public function fieldsInformation($post)
	{
		...
		$html .= '<option value="additional" '.($value === 'additional' ? 'selected' : null).'>Projetos Complementares</option>';
		$html .= '<option value="building" '.($value === 'building' ? 'selected' : null).'>Execução de Obras</option>';
    $html .= '<option value="security" '.($value === 'security' ? 'selected' : null).'>Segurança do Trabalho</option>';
		$html .= '</select>';
		$html .= '</td>';
		$html .= '</tr>';
		$html .= "</tbody></table>";
